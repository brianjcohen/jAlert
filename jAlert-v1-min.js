...
function jAlert(title,message,url,iframeSrc,iframeHeight,classes,id,hideOnClick,closeBtn,onClose){title=title||false;message=message||false;url=url||false;iframeSrc=iframeSrc||false;iframeHeight=iframeHeight||'600px';classes=classes||false;id=id||false;if(hideOnClick!==true){hideOnClick=false;}
...
div+="<div class='jContent'>"+content+"</div></div></div></div>";$('html, body').animate({scrollTop:0},'slow');var div=$(div);div.appendTo('body');div.show('fast');if(closeBtn){div.find('.closeAlert').on('click',function(){closeAlert(div,false,onClose);});}
if(hideOnClick){$(document).one('mouseup',function(e){closeAlert(div,false,onClose);});}
div.find('.autofocus:first').focus();return div;}
...
function staticAlert(title,message,classes,id,hideOnClick,closeBtn,onClose){return jAlert(title,message,false,false,false,classes,id,hideOnClick,closeBtn,onClose);}
function iframeAlert(title,iframeSrc,iframeHeight,classes,id,hideOnClick,closeBtn,onClose){return jAlert(title,false,false,iframeSrc,iframeHeight,classes,id,hideOnClick,closeBtn,onClose);}
function ajaxAlert(title,url,classes,id,hideOnClick,closeBtn,onClose){return jAlert(title,false,url,false,false,classes,id,hideOnClick,closeBtn,onClose);}
